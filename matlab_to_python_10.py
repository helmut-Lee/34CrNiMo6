...
path='E:/LTRANS/YECS_sarga_2020wnd1/output_apw_f1/'
path2='E:/LTRANS/YECS_sarga_2020wnd1/output_apw_f0/'
path3='E:/LTRANS/YECS_sarga_2020wnd1/output_apw_f0.5/'
path4='D:/output2016-2019/'
dlist, dlist2, dlist3=os.listdir(path), os.listdir(path2), os.listdir(path3)
# os.mkdir(path2+dlist2[10]+'/figure')
flist=sorted(glob.glob(path+dlist[10]+'/*.csv'))
flist2=sorted(glob.glob(path2+dlist2[10]+'/*.csv'))
flist3=sorted(glob.glob(path3+dlist3[10]+'/*.csv'))

A=pd.date_range('2019-04-01 12:00', periods=120, freq='+1d').strftime('%Y-%m-%d %H:%M') # fwd
# A=pd.date_range('2019-05-03 12:00', periods=180, freq='-1d').strftime('%Y-%m-%d %H:%M') # bwd
...
for i in range(round(len(flist)/24)):
    ...
    xt, xt2, xt3, yt, yt2, yt3=M(C[:,2], D[:,2], E[:,2], C[:,3] ,D[:,3], E[:,3])    
    ...
    plt.savefig(path2+dlist2[10]+'/figure/'+flist[(i*24)+1][64:76]+'.png',format='png',
